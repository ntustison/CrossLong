...
FreeSurfer, cortical surfaces are generated in} [@Li:2014aa] \textcolor{blue}{permitting vertex-wise 
quantitation of thickness and thickness change.  Application to early infants demonstrate
...
gray/white matter probability map.  The resulting mapping defines the diffeomorphicpath between a point 
on the gray/white matter interface and the gray matter boundary.   Cortical thickness values 
can then be assigned at each spatial location within the cortex by integrating along the 
...
\textcolor{blue}{
with the actual implementation provided in the class itk::DiReCTImageFilter available as 
...
values are expected but large variance values are not prohibited.  Even so results proved 
...
R interface to Stan was used to calculate the point estimates of the LME model
...
is measured with error (a common assumption in the measurement error literature [XX64],[XX65]), this bias is 
...
[@seber2012linear] \textcolor{blue}{(Chapter 3) states that effect estimator bias is not the only problem: 
...
through time included ([XX65], Section 3.3.1). The same authors state that this bias is 
...
sulcal depth, and gyrification) [@Shimony:2016aa] cannot be generated in a 
