...
\textcolor{blue}{Although the FreeSurfer longitudinal processing stream is perhaps 
...
\textcolor{blue}{permitting vertex-wise 
quantitation of thickness and thickness change.  Application to early infants 
in} [@Li:2014aa] \textcolor{blue}{further demonstrate
...
on measurement quality in terms of within-subject and between-subject variances which is known to
...
and less varied confidence/credible intervals for discerning cross-sectional effects. Similar to
\textcolor{black}{previously outlined} research,
we show that reorienting individual time point images to a
single-subject template has a favorable performance effect
...
[@Li:2014aa]\textcolor{blue}{) where "ground-truth" is known and, 
regression analysis of longitudinal trajectories of regional cortical 
...
\textcolor{black}{(e.g., Alzheimer's disease diagnosis)}. As such this
...
\textcolor{blue}{They concentrate mass near zero but have heavy tails, meaning small variance 
...
$\beta^i_k$ denotes the subject-specific slopes of cortical atrophy.}
For each region $k$,
...
programming language [@carpenter2016stan].  \textcolor{blue}{Specifically, the 
R interface to Stan was used to calculate the point estimates of the LME model (\ref{eq::lme1})
...
typical even in generalized linear models (Section 3.6) and continue to use the ratio as a 
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.9\textwidth]{../Data/RegionalPredictionPlots/accuracyTotal.png}
...
\textcolor{blue}{
The variance ratio is a generic desideratum for a statistical assessment
of performance over the set of possibile use cases.  Here we demonstrate two such possibilities
...
pipeline, we use those slope values to predict age and MMSE scores (cf Figure \ref{fig:mmsePrediction} for certain
cortical regions. We predict age for the regions selected in [@Tustison:2014ab]
...
the linear model 
\begin{equation}
Age_i \sim \Delta Y_i^k + Diagnosis_i 
\end{equation}
...
tested on the remaining 10\%.  This training/prediction scenario is repeated 100 times 
where the assignment to training or prediction data subsets is shuffled each time.
The results of this predictive evaluation is 
shown in Figure \ref{fig:agePrediction}.
}

\textcolor{blue}{
In Figures \ref{fig:variance_boxplots} and \ref{fig:brain_variance}, it is shown 
that the ANTsSST variance ratio is higher in most regions except for the left and entorhinal cortex
where the FSLong values are slightly higher.  Given the characteristics of the ADNI 
population and the fact that the entorhinal cortex is "ground zero" for AD pathology}
[@Yassa:2014aa]\textcolor{blue}{, 
we demonstrate that the higher variance ratio translates into better
MMSE prediction values for the FSLong pipeline. 
Thus, we evaluate the variance ratio in the context 
of the linear model 
\begin{equation}
MMSE_i \sim \Delta Y_i^k + Age_i 
\end{equation}
where $i$ denotes the $i^{th}$ subject, $\Delta Y_i^k$ is the atrophy rate for the $i^{th}$ subject
in the $k^{th}$ region of interest where the model is trained similarly to that for age
prediction.
}

\begin{figure}[ht!]
\centering
\includegraphics[width=0.6\textwidth]{../Data/RegionalPredictionPlots/accuracyEcTotal.png}
\caption{\textcolor{blue}{MMSE prediction error for each of the longitudinal processing pipelines
in the left and right entorhinal cortex.  Consistent with the variance ratio comparison,  
FSLong has a lower relative prediction error. }
}
...
capacity.  In these terms, ANTs SST outperformed all other pipeline variants
...
In addition, being voxel-based, the ANTs framework does have a couple 
of advantages not explored in this work but certainly to be utilized in
future research.  Specifically, the inherent voxel-based input/output processing
is conducive to voxel-based analysis strategies (e.g., Eigenanatomy}
 [@Kandel:2015aa]\textcolor{blue}{)
and straightforward extension to non-human research domains.
}
