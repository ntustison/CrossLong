...
\textcolor{red}{Based on the above ANTs pipeline descriptions, there are three
major variants for cortical thickness processing of longitudinal data.} We
denote these alternatives as:

* __ANTs Cross-sectional__ (or __ANTs Cross__).__ Process each subject's time point independently using the
  cross-sectional pipeline originally described in [@Tustison:2014ab].
* __ANTs Longitudinal-SST__ (or __ANTs SST__).__  Rigidly transform each subject to the SST and then
   segment and estimate cortical thickness in the space of the SST.
* __ANTs Longitudinal-native__ (or __ANTs Native__).__  Segment and estimate cortical thickness in the native space.

\textcolor{red}{For completeness, we also include a comparison with both the cross-section and
...
\textcolor{red}{Possible evaluation strategies could employ manual measurements in
the histological} [@rosas2002] \textcolor{red}{or virtual} [@kuperberg2003]
\textcolor{red}{domains but would require an inordinate labor effort for collection
...
\textcolor{red}{used FreeSurfer cortical thickness measurements across image
acquisition sessions to demonstrate improved reproducibility with the longitudinal
stream over the cross-sectional stream.  In} [@Klein:2017aa] \textcolor{red}{
...
For example, Figure 8} in [@Tustison:2014ab] \textcolor{red}{confirms what was found in} [@Klein:2017aa]
\textcolor{red}{which is that the range of ANTs cortical thickness values for a particular region
...
in} [@Tustison:2014ab] \textcolor{red}{demonstrated similarity in both ANTs and FreeSurfer
...
\textcolor{red}{A summary measure related to the ICC statistic} [@verbeke2009linear]
\textcolor{red}{is used to quantify the relative performance of these
...
\textcolor{red}{(e.g., Alzheimer's disease diagnosis)}. As such this will
...
### Entorhinal cortical thickness variability
...
the performance of the five processing approaches \textcolor{red}{(FS Cross, FS Long, ANTs Cross,
...
variability affect confidence in prediction and estimation \textcolor{red}{for these different pipelines}.
...
plots are available in the associated github repository} [@crossLong]\textcolor{red}{.  These
...
\caption{\textcolor{red}{Sample spaghetti plots for the longitudinal thickness values estimated
...
Longitudinal-native method dominates the others \textcolor{red}{across the majority of
regions}: its point estimates (posterior medians) are greater than those of the other
processing methods \textcolor{red}{except for the left and right EC values in
FreeSurfer Long (although there is significant overlap in the credible intervals
in those regions)}.  
These results also suggest that \textcolor{red}{longitudinal processing is to be
...
### ANTs-based entorhinal cortical thickness variability
...
structures, segmentation of the EC can be relatively difficult} [@price2010]\textcolor{red}{.
This segmentation complexity has led to EC-specific} [@Fischl:2009aa] \textcolor{red}{and related}
[@Augustinack:2013aa] \textcolor{red}{strategies for targeted regional processing.  
