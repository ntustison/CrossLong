...
\begin{align}
\Delta Y^k_{ij} & = Y_{i,bl}^k\, \beta_0 + AGE_{i,bl}\, \beta_1 + ICV_{i,bl}\, \beta_2 + APOE_i\, \beta_3 + GENDER_i \,\beta_4 \nonumber \\
           & + DIAGNOSIS_i\, \beta_5 + VISIT_{ij}\,\beta_6 + VISIT_{ij} \times DIAGNOSIS_i\,\beta_7  \nonumber \\
           &+ \alpha_i^k + \gamma_i^k + \epsilon_{ij}^k \, , 
\end{align}
where $\alpha_i^k$, $\gamma_i^k$, and $\epsilon_{ij}^k$  are independent, mean zero random variables representing individual-specific random intercepts, site-specific random intercepts, and residual errors, respectively.
