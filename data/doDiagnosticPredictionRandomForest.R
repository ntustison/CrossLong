...
#         }
  ...
  for( n in 1:length( slopeTypes ) )
    {
#     featureImp.df <- data.frame( Statistic = colnames( crossSlopeData )[grep( 'thickness', colnames( crossSlopeData))], Importance = featureImp[[n]] )

    featureImp.df <- data.frame( Statistic = names( featureImp[[n]][,1] ), Importance = as.numeric( featureImp[[n]][,1] )  )
    ...
    ggsave( file = paste( "~/Desktop/importanceRf", slopeTypes[n], p, ".pdf", sep = "" ), plot = vPlot, width = 4, height = 8 )
    }
  ...
  ggsave( filename = paste( "~/Desktop/accuracyRf", p, ".pdf", sep = "" ), plot = rmsePlot, width = 6, height = 6, units = 'in' )
...
#   > myAov
#   Call:
#      aov(formula = Accuracy ~ Pipeline, data = resultsData)
#
#   Terms:
#                   Pipeline Residuals
#   Sum of Squares  0.043555  4.673265
#   Deg. of Freedom        2      1497
#
#   Residual standard error: 0.05587265
#   Estimated effects may be unbalanced
#   > summary( myAov )
#                 Df Sum Sq  Mean Sq F value   Pr(>F)
#   Pipeline       2  0.044 0.021778   6.976 0.000965 ***
#   Residuals   1497  4.673 0.003122
#   ---
...
#   $Pipeline
#                      diff          lwr        upr     p adj
#   Long1-Cross 0.002798992 -0.005491197 0.01108918 0.7079593
#   Long2-Cross 0.012570462  0.004280273 0.02086065 0.0011242
#   Long2-Long1 0.009771470  0.001481281 0.01806166 0.0158742
