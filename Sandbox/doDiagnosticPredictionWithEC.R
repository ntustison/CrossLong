...
for( p in trainingPortions )
  ...
  for( n in 1:length( slopeTypes ) )
    ...
    predictorColumns <- sort( c( grep( "thickness", colnames( slopeDataList[[i]] ) ) ) )

    featureImp.df <- data.frame( Statistic = colnames( slopeDataList[[i]] )[which( colnames( slopeDataList[[i]] ) != 'DIAGNOSIS' & colnames( slopeDataList[[i]] ) != 'AverageAge' )],
